...
class SessionContainer extends Container
{
	...
	function load (&$authenticated, &$attributes, &$secure)
	{
		...
		if ($_SESSION['authenticated']) {
			// check session livetime
			if (!empty($_SESSION['session_lt_lastactiontime']) && !empty($_SESSION['session_lt_livetime'])) {
				//$diff = $_SESSION['session_lt_lastactiontime'] - time();
				//die($diff);
			}
			$_SESSION['session_lt_lastactiontime'] = time();
		}
